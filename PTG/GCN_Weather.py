...
class GCN(torch.nn.Module):
    def __init__(self):
        ...
        (GCNConv(17,64, aggr = 'max'),'x, edge_index, edge_weight -> x'),
        nn.ReLU(inplace = True),
        (nn.Dropout(0.25), 'x -> x')
        ])

        self.convA = Sequential('x, edge_index, edge_weight', [
        (GCNConv(17,64, aggr = 'add'),'x, edge_index, edge_weight -> x'),
        nn.ReLU(inplace = True),
        (nn.Dropout(0.2), 'x -> x')
        ])
...
class GCN(torch.nn.Module):
    def __init__(self):
        ...
        self.convM = Sequential('x, edge_index, edge_weight', [
        (GCNConv(17,16, aggr = 'max'),'x, edge_index, edge_weight -> x'),
        nn.ReLU(inplace = True),
        (nn.Dropout(0.1), 'x -> x')
        ])

        self.convA = Sequential('x, edge_index, edge_weight', [
        (GCNConv(17,16, aggr = 'add'),'x, edge_index, edge_weight -> x'),
        nn.ReLU(inplace = True),
        (nn.Dropout(0.1), 'x -> x')
        ])
