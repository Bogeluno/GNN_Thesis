...
class GCN(torch.nn.Module):
    def __init__(self):
        ...
        (GATConv(273,64, aggr = 'max', edge_dim = 1),'x, edge_index, edge_weight -> x'),
        nn.ReLU(inplace = True),
        (nn.Dropout(0.25), 'x -> x')
        ])

        self.convA = Sequential('x, edge_index, edge_weight', [
        (GATConv(273,64, aggr = 'add', edge_dim = 1),'x, edge_index, edge_weight -> x'),
        nn.ReLU(inplace = True),
        (nn.Dropout(0.2), 'x -> x')
        ])
...
class GCN(torch.nn.Module):
    def __init__(self):
        ...
        self.convM = Sequential('x, edge_index, edge_weight', [
        (GATConv(273,32, aggr = 'max', edge_dim = 1),'x, edge_index, edge_weight -> x'),
        nn.ReLU(inplace = True),
        (nn.Dropout(0.1), 'x -> x')
        ])

        self.convA = Sequential('x, edge_index, edge_weight', [
        (GATConv(273,32, aggr = 'add', edge_dim = 1),'x, edge_index, edge_weight -> x'),
        nn.ReLU(inplace = True),
        (nn.Dropout(0.1), 'x -> x')
        ])
...
class GCN(torch.nn.Module):
    def __init__(self):
        ...
        self.convM = Sequential('x, edge_index, edge_weight', [
        (GATConv(273,16, aggr = 'max', edge_dim = 1),'x, edge_index, edge_weight -> x'),
        nn.ReLU(inplace = True),
        (nn.Dropout(0.1), 'x -> x')
        ])

        self.convA = Sequential('x, edge_index, edge_weight', [
        (GATConv(273,16, aggr = 'add', edge_dim = 1),'x, edge_index, edge_weight -> x'),
        nn.ReLU(inplace = True),
        (nn.Dropout(0.1), 'x -> x')
        ])
