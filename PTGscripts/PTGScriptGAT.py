...
class GCN(torch.nn.Module):
    def __init__(self):
        ...
        self.seq = Sequential('x, edge_index, edge_weight', [
            (GATConv(264,32, edge_dim = 1),'x, edge_index, edge_weight -> x'),
            nn.ReLU(inplace = True),
            (nn.Dropout(0.1), 'x -> x'),
            (GATConv(32,16,edge_dim = 1),'x, edge_index, edge_weight -> x'),
            nn.ReLU(inplace = True),
            (nn.Dropout(0.1), 'x -> x'),
            (GATConv(16,1,edge_dim = 1),'x, edge_index, edge_weight -> x')
        ])


    def forward(self, data):
        x, edge_index, edge_weight, num_nodes = data.x, data.edge_index, data.edge_attr, data.num_nodes
...
class GCN(torch.nn.Module):
    def __init__(self):
        ...
        self.seq = Sequential('x, edge_index, edge_weight', [
            (GATConv(264,64, edge_dim = 1),'x, edge_index, edge_weight -> x'),
            nn.ReLU(inplace = True),
            (nn.Dropout(0.1), 'x -> x'),
            (GATConv(64,48, edge_dim = 1),'x, edge_index, edge_weight -> x'),
            nn.ReLU(inplace = True),
            (nn.Dropout(0.1), 'x -> x'),
            (GATConv(48,1, edge_dim = 1),'x, edge_index, edge_weight -> x')
        ])
...
class GCN(torch.nn.Module):
    def __init__(self):
        ...
        self.seq = Sequential('x, edge_index, edge_weight', [
            (GATConv(264,96, edge_dim=1),'x, edge_index, edge_weight -> x'),
            nn.ReLU(inplace = True),
            (nn.Dropout(0.1), 'x -> x'),
            (Linear(96,32),'x -> x'),
            nn.ReLU(inplace = True),
