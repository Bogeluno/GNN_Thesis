...
from torch_geometric.nn import Sequential, GCNConv, SGConv, GraphConv, TopKPooling, Linear
...
sys.stdout = open("PTG_withD.txt", "w")
...
class GCN(torch.nn.Module):
    def __init__(self):
        ...
        self.seq = Sequential('x, edge_index, edge_weight', [
            (GCNConv(265,32, aggr = 'add'),'x, edge_index, edge_weight -> x'),
...
class GCN(torch.nn.Module):
    def __init__(self):
        ...
        self.seq = Sequential('x, edge_index, edge_weight', [
            (GCNConv(265,32, aggr = 'max'),'x, edge_index, edge_weight -> x'),
            nn.ReLU(inplace = True),
            (nn.Dropout(0.1), 'x -> x'),
            (Linear(32,16),'x -> x'),
            nn.ReLU(inplace = True),
...
class GCN(torch.nn.Module):
    def __init__(self):
        ...
        self.seq = Sequential('x, edge_index, edge_weight', [
            (GCNConv(265,96, aggr = 'max'),'x, edge_index, edge_weight -> x'),
            nn.ReLU(inplace = True),
            (nn.Dropout(0.1), 'x -> x'),
            (Linear(96,48),'x -> x'),
            nn.ReLU(inplace = True),
...
class GCN(torch.nn.Module):
    def __init__(self):
        ...
        (GCNConv(265,48, aggr = 'max'),'x, edge_index, edge_weight -> x'),
        nn.ReLU(inplace = True),
        (nn.Dropout(0.25), 'x -> x')
        ])

        self.convA = Sequential('x, edge_index, edge_weight', [
        (GCNConv(265,48, aggr = 'add'),'x, edge_index, edge_weight -> x'),
        nn.ReLU(inplace = True),
        (nn.Dropout(0.25), 'x -> x')
        ])
